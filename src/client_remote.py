...
def get_nvidia_smi():
    try:
        p = subprocess.run(["nvidia-smi"], stdout=subprocess.PIPE, timeout=1)
...
def create_client_strategy(backend, cmd_name, client_args):
    ...
    if client_args:
        # Flatten dict like {"--foo": ["bar"], "--baz": ["a", "b"]}
        flat_args = [cmd_name] + list(reduce(lambda x, y: x + y, client_args.items()))
    else:
        flat_args = [cmd_name]

    # If args may contain shell-style quoting, normalize them safely
    parser_args = normalize_args(flat_args)
...
def normalize_args(args):
    """
    Safely apply shell-style parsing to arguments without invoking a shell.
    """
    if not args:
        return []

    # Join into a single string, then split like a shell would
    arg_str = " ".join(args)
